...
| `incarichi-nominativi-shard` | Incarichi nominativi: fonti estese | 39.685 | Base per le relazioni persona → incarico → ente. Colonne verificate via API: `ente`, `ipa`, `tipo`, `nominativo`, `oggetto`, `cig`, `importo_euro`, `data`, `fonte_url`, `note`. Sovrapposizione parziale con `nominativi-incarichi`: mai sommare senza riconciliazione |
...
| `affidamenti-diretti` | Affidamenti diretti | 6.506 | Base principale per l'analisi degli aggiudicatari. Colonne verificate via API: `ente`, `ipa`, `cig`, `contraente`, `importo`, `oggetto`, `data`, `url`; periodo di riferimento 2024–2026 |
...
| `nominativi-incarichi` | Incarichi nominativi | 1.633 | Può sovrapporsi a `incarichi-nominativi-shard`: vedi LIMITI |
...
Ogni riga restituita dall'API porta con sé provenienza completa: `id` del record, hash SHA-256 della riga sorgente (`sourceRowSha256`), URL dell'atto originale (`sourceUrls`) e numero di riga nella fonte. Questi elementi alimentano direttamente i campi obbligatori dello schema delle relazioni.
...
- Rilascio DVNS utilizzato per le prime verifiche via API: `checkedAt` 2026-08-23 (release set hash `86dc9a6d…7d8928`, dichiarato nella paginazione delle risposte).
